...
public class SENSE_INFO_CRAWLING {
	...
	public List<String> SenseInfo() {
		...
		//이동을 원하는 url
		String url = "https://mypetlife.co.kr/popular-posts/?filter=dog";
		...
		List<WebElement> el1 = driver.findElements(By.className("content"));
		
		for (int i = 0; i < el1.size(); i++) {
			//nav 클래스의 객체 중 "뉴스"라는 텍스트를 가진 WebElement를 클릭한다.
			if(el1.get(i).getText().equals("강아지")) {
				el1.get(i).click();
				break;
			}
		}
		...
		//iT/과학뉴스를 담은 div
		WebElement el2 = driver.findElement(By.className("bl-post-list"));
		
		//div속에서 strong태그를 가진 모든 element를 받아온다.
		List<WebElement> el3 = el2.findElements(By.className("post-title"));
		
		List<String> news = new ArrayList<String>();
		
		for (int i = 0; i < el3.size(); i++) {
			//뉴스의 제목을 모두 출력한다.
			news.add(el3.get(i).getText());
		}
