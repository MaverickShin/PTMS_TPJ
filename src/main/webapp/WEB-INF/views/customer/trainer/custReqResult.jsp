						...
						<c:forEach var="dtos" items="${dto}" varStatus="status">
							<div class="col-md-12" style="background-color: #f5f5f5;">
						        ...
						        <c:if test ="${dtos.TQ_ST == 0}">
						        	<input type="button" value="요청취소"  onclick="window.location='cancelRequestTraining?TQ_CD=${dtos.TQ_CD}'">
						      	</c:if>
						      	<c:if test="${dtos.TQ_ST == 1}">
						      		<input type="button" value="결제하기" onclick="window.location='/tpj/pay/request?item_name=펫 훈련 결제&price=${dtos.TQ_FEE}&primarykey=${dtos.TQ_CD}'">
						      	</c:if>
						      	<c:if test="${dtos.TQ_ST == 3}">
						      		<input type="button" value="매칭확정하기" onclick="window.location='updateTrainingComplete?TQ_CD=${dtos.TQ_CD}'">
						      	</c:if>
						      	<c:if test="${dtos.TQ_ST == 4}">
							        ...
							        <c:if test="${reviewCheckCnt[status.index] != 1}">
							        	<input type="button" value="후기쓰기" onclick="window.location='writeTrainingReview?TQ_CD=${dtos.TQ_CD}&TG_ID=${dtos.TA_CD}'">
							        </c:if>
						        </c:if>
						    </div>
					    </c:forEach>
